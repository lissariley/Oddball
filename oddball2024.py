...
def showExpInfoDlg(expInfo):
    ...
    # return expInfo
    return expInfo
print('about to get dialog*************************************************************************************************')
expInfo = showExpInfoDlg(expInfo=expInfo) #we're running this early, before finishing global variable setup, so we can know whether the user wants tracking/scanning or not
print('got dialog*************************************************************************************************')


#make eyelink setup conditional on information from expInfo
...
# create a folder for the current testing session in the "results" folder
    ...
    print('I am setting up EDF stuff *****************************************************************************')
    #%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='C:\\Users\\aclab\\Desktop\\Oddball\\oddball_buildup\\EyeLinkMRIdemo_Builder.py',
...
def setupWindow(expInfo=None, win=None):
    ...
    if win is None:
        ...
        win.color = '[0.6541, 0.6768, 0.6473]'
        win.colorSpace = 'rgb'
        win.backgroundImage = ''
...
def run(expInfo, thisExp, win, inputs, globalClock=None, thisSession=None):
    ...
    welcometext = visual.TextStim(win=win, name='welcometext',
        text='Welcome to the Oddball Task!\nRemember, your job is to: \npress the space bar \nwhen you see the LARGE circle.',
        font='Open Sans',
    ...
    while continueRoutine:
        ...
        if keyTrigger.status == STARTED and not waitOnFlip:
            if int(expInfo['scanning'])==0:
                theseKeys = keyTrigger.getKeys(keyList=['space'], ignoreKeys=["escape"], waitRelease=False)
            elif int(expInfo['scanning'])==1:
    ...
    if int(expInfo['practice'])==0:
        trials = data.TrialHandler(nReps=1.0, method='sequential', 
            extraInfo=expInfo, originPath=-1,
            trialList=data.importConditions('conditions.csv'),
            seed=None, name='trials')
    elif int(expInfo['practice'])==1:
        trials = data.TrialHandler(nReps=1.0, method='sequential', 
            extraInfo=expInfo, originPath=-1,
            trialList=data.importConditions('practice_conditions_longer.csv'),
            seed=None, name='trials')
    ...
    for thisTrial in trials:
        ...
        while continueRoutine:
            ...
            # check keyboard events and then check to see if abort key combination (Ctrl-C) pressed
                ...
                keyPressList = kb.getKeys(keyList = ['escape'], waitRelease = False, clear = False)
                ...
                if ('escape') in keyPressNameList:
                    el_tracker.sendMessage('terminated_by_user')
                    terminate_task(genv,edf_file,session_folder,session_identifier)
            #check for key releases
                #keyReleaseList = kb.getKeys(keyList = ['lctrl','rctrl','c'], waitRelease = True, clear = False)
                keyReleaseList = kb.getKeys(keyList = ['escape'], waitRelease = True, clear = False)
